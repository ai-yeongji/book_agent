...
import { fetchKyoboBestsellers, generateSocialContent, generateImagesForScenes, generateImage } from './services/geminiService';
  ...
  const handleGenerate = async (type: 'instagram_post' | 'reels_script') => {
    ...
    try {
      ...
      if (type === 'reels_script' && result.scenes && result.scenes.length > 0) {
        ...
        generateImagesForScenes(result.scenes, coverUrl).then((updatedScenes) => {
      ...
      if (type === 'instagram_post' && result.imagePrompt) {
        setIsGeneratingImages(true);
        generateImage(result.imagePrompt, coverUrl).then((url) => {
          setGeneratedResult(prev => {
            if (!prev) return null;
    ...
    <Layout>
      ...
      {state === AppState.IDLE && (
        <div className="flex flex-col items-center justify-center py-20 text-center animate-fade-in">
          ...
          <h2 className="text-3xl font-bold text-slate-900 mb-4">
            Turn Bestsellers into Viral Content
          </h2>
          <p className="text-slate-500 max-w-md mb-8 leading-relaxed">
            Automatically fetch top books from Kyobo, create scripts, and visualize storyboards with <span className="font-semibold text-indigo-600">AI Generated Scenes</span>.
          </p>
          <button
          ...
          >
            ...
            <span>Scan Kyobo Bestsellers</span>
          </button>
        </div>
      )}
      ...
      {state === AppState.SEARCHING && (
        <div className="flex flex-col items-center justify-center py-32 animate-pulse">
          ...
          <p className="text-slate-500 text-sm mt-2">Powered by Google Search Grounding</p>
        </div>
      )}
      ...
      {(state === AppState.SELECTING || state === AppState.GENERATING) && (
        <div className="space-y-8 animate-fade-in-up">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold text-slate-900">Top 5 Bestsellers</h2>
              <p className="text-slate-500 text-sm mt-1">Select a book to create content</p>
            </div>
